...
import GinaTech02.Cnstock_dao as dbo
...
class stock_cal(metaclass=ABCMeta):
...
class cnstock_cal(stock_cal):
    ...
    def cal_turnoverratio(self):
        dailydb = dbo.cnstock_daily_dao()
        list = dailydb.get_turnoverratio_list(self.__symbol)
        if len(list) == self.__datasize:
            return list
        else:
            print("turnover rate leng != datasize")
            return np.zeros(self.__datasize, dtype=float)
        pass
